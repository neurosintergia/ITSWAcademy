<div>
                     ...
                     @if ($lesson->id == $item->id)
                       ...
                       <form wire:submit.prevent="update">
                         ...
                         <div class="flex items-center">
                             ...
                             <input wire:model="lesson.name" class="form-input bg-blue-100 rounded-lg w-full">
                           </div>
                           @error('lesson.name')
                           ...
                           <div class="flex items-center mt-4">
                             ...
                             <input wire:model="lesson.url" class="form-input bg-blue-100 rounded-lg w-full">
                           </div>
                           @error('lesson.url')
                           <span class="text-xs text-red-500">{{$message}}</span>
                           @enderror
    ...
    <div class="mt-4 " x-data="{open:false}">
     <a x-show="!open" x-on:click="open = true"  class="flex font-bold text-blue-900 items-center cursor-pointer">
         <i class="far fa-plus-square text-2xl text-red-500 mr-4"></i>Agregar nueva lección
     </a>
     <div class="card" x-show="open">
         <div class="card-body">
             ...
             <div class="mb-4">
                 <div class="flex items-center">
                     <label class="w-32">Nombre:</label>
                     <input wire:model="name" class="form-input  bg-blue-100 rounded-lg w-full">
                   </div>
                   @error('name')
                   ...
                   <div class="flex items-center mt-4">
                     <label class="w-32">URL:</label>
                     <input wire:model="url" class="form-input bg-blue-100 rounded-lg w-full">
                   </div>
                   @error('url')
                   <span class="text-xs text-red-500">{{$message}}</span>
                   @enderror
