<section class="mt-4">
    <h1 class="font-bold text-3xl text-gray-800 mb-2">Comentarios</h1>

    @can('enrolled', $course)
       <article class="mb-4">

    
            <textarea wire:model="comment" class="form-input w-full" rows="3" placeholder="Ingrese un comentario"></textarea>   
             @error('comment')
            ...
            <div class="flex">
                ...
                <button class="btn btn-primary mr-3" wire:click="store">Comentar</button>
            </div>
  ...
  <div class="card">
      <div class="card-body">
 <p class="text-gray-800 text-xl">{{$course->chats->count()}} Valoraciones</p>

          @foreach ($course->chats as $chat)
              <article class="flex mb-4 text-gray-600">
                    <figure class="mr-4">
                        <img class="h-12 w-12 object-cover rounded-full shadow-lg" src="{{$chat->user->profile_photo_url}}" alt="">
                    </figure>

                    <div class="card flex-1">
                        <div class="card-body bg-gray-100">
                            <p><b>{{$chat->user->name}} {{$chat->user->created_at->format('d/m/Y')}}</b><i class="fas fa-star text-yellow-300"></i>({{$chat->rating}})</p>
                            {{$chat->comment}}
                        </div>
                    </div>
              </article>
